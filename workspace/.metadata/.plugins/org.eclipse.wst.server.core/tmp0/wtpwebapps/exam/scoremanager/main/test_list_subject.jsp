...
<c:import url="/common/base.jsp">
    ...
    <c:param name="content">
        <section class="me-4">
                        ...
                        <div class="col-6"> <!-- 学生番号入力欄 -->
                            <div class="row align-items-center">
                                <div class="col-9">
                                    ...
                                    <input class="form-control" type="text" id="student-name-input" name="f4" placeholder="学生番号を入力してください" maxlength="10" value="${f4}" required style="color: black;" />
            ...
            <c:choose>
                <c:when test="${!empty tests && !empty tests[0].points}">
                    <p>科目:${f5.name}</p>
                    <table class="table table-hover">
                        <tr>
                            ...
                            <th>学籍番号</th>
                            <th>氏名</th>
                            <c:forEach var="entry" items="${tests[0].points.entrySet()}">
                                <th>${entry.key}回</th>
                            </c:forEach>
                        </tr>
                        <c:forEach var="test" items="${tests}">
                            <tr>
                                ...
                                <c:forEach var="entry" items="${tests[0].points.entrySet()}">
                                    <td>
                                        <c:choose>
                                            <c:when test="${empty test.points[entry.key]}">-</c:when>
                                            <c:otherwise>${test.points[entry.key]}</c:otherwise>
                                        </c:choose>
                                    </td>
                                </c:forEach>
                            </tr>
